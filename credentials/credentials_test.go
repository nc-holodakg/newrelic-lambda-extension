...
func TestGetLicenseKeyParameterName(t *testing.T) {
	parameterName := getLicenseKeyParameterName(&config.Configuration{})
	assert.Equal(t, "", parameterName)

	var testParameterName = "testParameterName"
	var conf = &config.Configuration{LicenseKeyParameterName: testParameterName}
...
func TestIsParameterConfigured(t *testing.T) {
	ctx := context.Background()
	assert.False(t, IsParameterConfigured(ctx, &config.Configuration{}))

	OverrideSSM(mockSSM{
		validParameters: []string{"testParameterName"},
	})
	assert.True(t, IsParameterConfigured(ctx, &config.Configuration{
		LicenseKeyParameterName: "testParameterName",
	}))
...
func TestGetNewRelicLicenseKey(t *testing.T) {
	table := []struct {
		...
		{
			Name: "loads license key from environment variable if configured secret is unavailable",
			Conf: config.Configuration{
				LicenseKeySecretId: "testSecretName",
			},
			SecretsManager: mockSecretManager{},
		...
		{
			Name: "loads license key from environment variable if configured parameter is unavailable",
			Conf: config.Configuration{
				LicenseKeyParameterName: "testParameterName",
			},
			SSM: mockSSM{},
		...
		{
			Name: "returns error if configured secret is unavailable and no fallback is available",
			Conf: config.Configuration{
				LicenseKeySecretId: "testSecretName",
			},
			SecretsManager: mockSecretManager{},
		...
		{
			Name: "returns error if configured parameter is unavailable and no fallback is available",
			Conf: config.Configuration{
				LicenseKeyParameterName: "testParameterName",
			},
			SSM: mockSSM{},
